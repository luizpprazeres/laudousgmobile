...
## O catálogo × a evolução do renderer

**Descoberto no port para a `main`, em 11/08.** O catálogo foi escrito e
validado contra o renderer de 09/08. Desde então a `main` ganhou três recursos
no obstétrico que ele **não conhece**:

| Recurso | Flag |
|---|---|
| Golf ball (foco ecogênico intracardíaco) | `GOLF_BALL_SNIPPET` |
| Sanity de IG | `OBST_IG_SANITY` |
| Biometria determinística | `OBST_BIOMETRIA_DET` |

A equivalência **continua valendo** — 3840/3840 contra o renderer atual da
`main` — porque esses recursos ficam desligados no teste e a lógica-base não
mudou. Mas se algum deles estiver **ligado em produção**, o catálogo produziria
um laudo sem aquele recurso.

Por isso `pipeline/renderer.ts` tem uma trava explícita: o catálogo só assume
quando os três estão desligados e o estilo é clássico. Perder um recurso em
silêncio seria pior que não ter catálogo nenhum.
