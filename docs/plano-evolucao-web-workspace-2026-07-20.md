...
O contrato local de sugestões foi implementado em seguida. Quando o médico já alterou o texto e os campos determinísticos produzem uma nova versão, o Workspace não substitui o laudo: calcula uma comparação por blocos, mostra o texto atual e o texto proposto e exige aceitar ou rejeitar. Rejeitar preserva o documento e reconhece a nova base; aceitar guarda a versão anterior e oferece desfazer. Esse mesmo contrato será a porta de entrada futura para propostas do agente e do celular, evitando caminhos paralelos de mutação clínica.

### Prioridade seguinte: celular como dispositivo de entrada

O agente de IA dentro do web deixou de ser a prioridade imediata. O Workspace web passa a ser a estação que recebe, organiza e aplica dados; o celular concentra a captura por texto, medidas, voz e imagem. O primeiro fluxo vertical usa o aplicativo atual: o computador cria um código de sessão válido por até 10 horas, o celular autenticado na mesma conta pareia e envia cartões de texto ou medidas, e o web exige uma ação explícita de aplicar ou descartar.

Essa sessão usa `workspace_sessions` e `workspace_inputs`, separadas dos `room_tokens` da Sala do Auxiliar. O canal começa com polling isolado a cada dois segundos e eventos idempotentes. Voz e OCR deverão produzir o mesmo envelope de entrada depois do processamento no celular/backend; imagens brutas e áudio não entram no banco de eventos. A flag `WEB_MOBILE_COMPANION` mantém toda a integração dormente até a migration e os endpoints estarem publicados.
