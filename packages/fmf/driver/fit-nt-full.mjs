...
const KEYS = ['a0', 'a1', 'b0', 'b1', 'b2', 'm1', 'sd', 'sdB', 'p21', 'm21', 'sdT21', 'p18', 'm18', 'sdT18', 'p13', 'm13', 'sdT13']
...
function lrs(P, nt, crl) {
  ...
  const tl = NT_TLIMITS.get(Math.round(crlR * 10)) ?? 1.01; const lt = Math.log10(tl)
...
const loss = x => { const P = toP(x); for (const k of ['p21', 'p18', 'p13']) if (P[k] <= 0.2 || P[k] >= 0.9995) return 1e9; for (const k of ['sd', 'sdB', 'sdT21', 'sdT18', 'sdT13']) if (P[k] <= 0.02 || P[k] > 0.7) return 1e9; let s = 0; for (const q of pts) { const l = lrs(P, q.nt, q.crl); if (!(l.t21 > 0 && l.t18 > 0 && l.t13 > 0)) return 1e9; s += (Math.log(l.t21) - Math.log(q.lr21)) ** 2; s += (Math.log(q.w18 * l.t18 + q.w13 * l.t13) - Math.log(q.lr1813)) ** 2 } return s / (2 * pts.length) }
...
const x0 = KEYS.map(k => NT_MIX[k]); const step = KEYS.map(k => /^p/.test(k) ? 0.02 : /^sd/.test(k) ? 0.02 : k === 'a0' ? 0.2 : k === 'a1' ? 0.01 : k === 'b1' ? 0.005 : k === 'b2' ? 0.00005 : 0.05)
...
const P = toP(best.x); console.log('ajuste:', JSON.stringify(Object.fromEntries(KEYS.map(k => [k, Number(P[k].toFixed(5))]))))
...
writeFileSync('nt-mix-fit.json', JSON.stringify({ rms: Math.sqrt(best.f), params: Object.fromEntries(KEYS.map(k => [k, P[k]])) }, null, 1))
