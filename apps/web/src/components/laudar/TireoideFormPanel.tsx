...
function FieldLabel({ children }: { children: React.ReactNode }) {
  return (
    <span className="mb-1.5 block font-mono text-[9.5px] font-semibold uppercase tracking-[0.14em] text-gray-400">
      {children}
    </span>
  )
}
    ...
    <div className="space-y-2.5">
      <section className="rounded-xl border border-gray-200 bg-white px-3.5 py-3 shadow-sm">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h3 className="font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-gray-500">Nódulos</h3>
      ...
      {state.nodulos.map((nodulo, index) => (
        <section key={nodulo.id} className="rounded-xl border border-gray-200 bg-white px-3.5 py-3 shadow-sm">
          <div className="mb-3 flex items-center justify-between gap-3">
            <h3 className="font-barlow text-lg font-bold tracking-tight text-gray-900">Nódulo {index + 1}</h3>
            <button
              type="button"
...
export function TireoideFormPanel({ section, state, onChange }: Props) {
  ...
  return (
    <section className="rounded-xl border border-gray-200 bg-white px-3.5 py-3 shadow-sm">
      <p className="text-[13px] text-gray-500">Selecione uma seção da tireoide.</p>
    </section>
  )
}
