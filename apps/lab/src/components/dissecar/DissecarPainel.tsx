...
type Props = {
  ...
  /** A SYSTEM MESSAGE, não o modelo de IA — é ela que diz o caminho. */
...
export function DissecarPainel({ laudo, structuredOutput, systemMessage }: Props) {
  ...
  const disponivel = procedenciaDisponivel(systemMessage, temStructured);
  const modo = modoDe(systemMessage);
