...
export async function GET(req: Request) {
  ...
  // A main usa o mecanismo INLINE de blocos condicionais — quatro predicados
  // por categoria, que é o que está no ar hoje. A branch de trabalho tem uma
  // generalização (`resolveConditionalPromptBlocks`) que ainda NÃO foi portada,
  // de propósito: ela refatora código quente sem flag.
  //
  // O Lab tem de mostrar o prompt que a PRODUÇÃO monta, não o que outra branch
  // montaria. Por isso aqui reproduzimos a detecção real, com as mesmas
  // funções que o writer usa.
  const cond = {
    hasAnexial: category === "PELVE_FEMININA" && hasAnexialMention(rawInput),
    hasPlacenta:
      (category === "OBSTETRICA" || category === "DOPPLER_OBSTETRICO") &&
      hasPlacentaMention(rawInput),
    hasHashimoto: category === "TIREOIDE" && hasHashimotoMention(rawInput),
    hasPolipo: category === "ABDOMEN_TOTAL" && hasPolipoMention(rawInput),
  };
  const condicionais: string[] = [
    cond.hasAnexial ? "PELVE_ANEXIAL" : "",
    cond.hasPlacenta ? "PLACENTA" : "",
    cond.hasHashimoto ? "TIREOIDE_HASHIMOTO" : "",
    cond.hasPolipo ? "ABDOMEN_POLIPO" : "",
  ].filter((x) => x !== "");
