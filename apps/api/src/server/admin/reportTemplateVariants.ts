...
export const CreateVariantBodySchema = z.object({
  ...
  // Só variantes de ESTILO entram no picker de preferências (DET-4 follow-up).
  preference_eligible: z.boolean().optional(),
...
export function toVariant(row: VariantRow) {
  return {
    ...
    version: row.version,
    status: row.status,
    preference_eligible: row.preferenceEligible,
