...
// TIREOIDE e7ab387b: "estímulo" = istmo (istmo ficava em branco).
{
  const out = normalizeAsrClinical("estímulo medindo 1.4 por 0.3 por 1.3 com volume de 0.3");
  check("estímulo → istmo", /^istmo medindo 1\.4/.test(out), out);
}
// PELVE 900c411c: "na ecoeca" / "ecoeca" = anecoica.
{
  check("'na ecoeca' → anecoica", normalizeAsrClinical("imagem na ecoeca com forma triangular") === "imagem anecoica com forma triangular");
  check("'ecoeca' → anecoica", /anecoica/.test(normalizeAsrClinical("imagem ecoeca")));
}
// PELVE 900c411c: "miolétrico" = miométrio.
{
  check("miolétrico → miométrio", normalizeAsrClinical("com o miolétrico residual de 0.5") === "com o miométrio residual de 0.5");
}
// CONTROLE: texto clínico legítimo NÃO é tocado (byte-idêntico).
{
  const raw = "Lobo direito medindo 4.3 por 1.9, imagem hipoecoica no miométrio, anecoica com septos.";
  check("texto legítimo intocado", normalizeAsrClinical(raw) === raw, normalizeAsrClinical(raw));
}
