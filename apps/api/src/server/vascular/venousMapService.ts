...
export async function extractVenousMap(
  ...
  try {
    // Chave DEDICADA ao desenho (per-segmento) — distinta do writer
    // DOPPLER_VENOSO_MMII (texto do laudo). Ver renderer/extraction.ts.
    const res = await extract("DOPPLER_VENOSO_MMII_SCHEME", rawInput, signal);
